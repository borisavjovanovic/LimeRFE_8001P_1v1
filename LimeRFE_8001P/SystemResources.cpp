...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/./" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/.");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("https://downloads.myriadrf.org/project/limesuite/./" + name);
