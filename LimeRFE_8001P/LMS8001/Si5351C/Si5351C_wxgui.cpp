...
lms8_Si5351C_wxgui::lms8_Si5351C_wxgui(wxWindow *parent, wxWindowID id, const wxString &title, const wxPoint &pos, const wxSize &size, int styles, wxString idname)
	: Si5351C_wxgui_view(parent, id, title, pos, size, styles)
{
}

void lms8_Si5351C_wxgui::Initialize(Si5351C *pModule)
{
	m_pModule = pModule;
}

void lms8_Si5351C_wxgui::OnbtnLoadFileClick(wxCommandEvent &event)
{
...
void lms8_Si5351C_wxgui::OnbtnConfigureClockClick(wxCommandEvent &event)
{
	double refFreq;
	double freq;
...
void lms8_Si5351C_wxgui::OnbtnResetToDefaultsClick(wxCommandEvent &event)
{
	m_pModule->Reset();
	m_pModule->UploadConfiguration();
}

void lms8_Si5351C_wxgui::OnbtnReadStatusClick(wxCommandEvent &event)
{
...
LOS:       %i    LOS_STKY:       %i",
									 stat.sys_init, stat.sys_init_stky, stat.lol_b, stat.lol_b_stky, stat.lol_a, stat.lol_a_stky, stat.los, stat.los_stky);
	lblStatus->SetLabel(text);
}

void lms8_Si5351C_wxgui::OnbtnClearStatusClick(wxCommandEvent &event)
{
