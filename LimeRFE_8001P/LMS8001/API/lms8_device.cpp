...
//#include "IConnection.h"
#include "../lms8001/connectionManager/IConnection.h"
//#include "dataTypes.h"
//#include "MCU_BD.h"
...
#include "../LMSBoards.h"
//#include "Lms8Generic.h"
//#include "lmsComms.h"


LMS8_Device* LMS8_Device::CreateDevice()
{
    ...
    LMS8_Device* device = new LMS8_Device(conn);

    return device;
}

LMS8_Device* LMS8_Device::CreateDevice(int i)
{
    LMScomms* conn = new LMScomms();
    conn->RefreshDeviceList();
...
/*
//namespace lime {
//    LMS8_Device* LMS8_Device::CreateDevice(const lime::ConnectionHandle& handle, LMS8_Device* obj)
    LMS8_Device* LMS8_Device::CreateDevice(const lime::ConnectionHandle& handle)
    {
        LMS8_Device* device;
//        if (obj)
//        {
...
//        device = new LMS8_Generic(conn, obj);
        device = new LMS8_Device(conn);
...
{
    connection = conn;
    lms = new LMS8001(conn);
}

/*
//    LMS8_Device::LMS8_Device(LMS8_Device* obj) : connection(nullptr), lms_chip_id(0), fpga(nullptr), limeRFE(nullptr)
    LMS8_Device::LMS8_Device(LMS8_Device* obj) : connection(nullptr)
    {
        if (obj != nullptr)
        {
...
//        lime::ConnectionRegistry::freeConnection(connection);
 ...
 int LMS8_Device::ReadLMSReg(uint16_t address, int ind) const
{
    return lms->SPI_read(address & 0xFFFF);
}

int LMS8_Device::WriteLMSReg(uint16_t address, uint16_t val, int ind) const
{
    return lms->SPI_write(address & 0xFFFF, val);
}

LMScomms* LMS8_Device::GetConnection()
{
    return connection;
}

lms_dev_info_t* LMS8_Device::GetInfo()
{
...
//        strncpy(devInfo.firmwareVersion, info.firmware.c_str(), sizeof(devInfo.firmwareVersion) - 1);
//        strncpy(devInfo.hardwareVersion, info.hardware.c_str(), sizeof(devInfo.hardwareVersion) - 1);
//        strncpy(devInfo.protocolVersion, info.protocol.c_str(), sizeof(devInfo.protocolVersion) - 1);
        sprintf(devInfo.firmwareVersion, "%d", info.firmware);
        sprintf(devInfo.hardwareVersion, "%d", info.hardware);
        sprintf(devInfo.protocolVersion, "%d", info.protocol);
    }
    return &devInfo;
}

LMS8001* LMS8_Device::GetLMS() const
{
    return lms;
}
...
LMS7_Device::Range LMS7_Device::GetRateRange(bool, unsigned) const
{
    return Range(100e3, 61.44e6);
}

std::vector<std::string> LMS7_Device::GetPathNames(bool dir_tx, unsigned) const
{
    if (dir_tx)
