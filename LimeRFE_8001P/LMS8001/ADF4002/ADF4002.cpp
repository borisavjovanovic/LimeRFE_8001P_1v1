 ...
 */
void lms8_ADF4002::GetConfig(unsigned char data[12])
{
    MakeData();
...
void lms8_ADF4002::SetFrefFvco(double Fref, double Fvco, int &rcount, int &ncount)
{
    txtFref = Fref;
    txtFvco = Fvco;
    CalculateRN();
...
void lms8_ADF4002::SetReferenceCounterLatch(int Ldp, int Abw, int refCounter)
{
    cmbLDP = Ldp;
    cmbABW = Abw;
    txtRCnt = refCounter;
...
void lms8_ADF4002::SetNCounterLatch(int CPgain, int NCounter)
{
    cmbCPG = CPgain;
    txtNCnt = NCounter;
}

void lms8_ADF4002::SetFunctionLatch(int currentSetting1, int currentSetting2, int timerCounter, int fastLock, int muxoutControl)
{
...
void lms8_ADF4002::SetFunctionLatchRgr(int pdPol, int pd1, int pd2, int counterReset, int cpState)
{
    rgrPD1_f = pd1;
    rgrPD2_f = pd2;
    rgrPDP_f = pdPol;
...
void lms8_ADF4002::SetInitializationLatch(int currentSetting1, int currentSetting2, int timerCounter, int fastLock, int muxoutControl)
{
...
void lms8_ADF4002::SetInitializationLatchRgr(int pdPol, int pd1, int pd2, int counterReset, int cpState)
{
    rgrPD1_i = pd1;
    rgrPD2_i = pd2;
    rgrPDP_i = pdPol;
...
// msavic 160530
void lms8_ADF4002::Initialize(LMScomms *pSerPort)
{
    ctrPort = pSerPort;
}

liblms8_status lms8_ADF4002::UploadReset()
{
    SetDefaults();
    // The intention is to put it into reset and 3-state, so we make sure it does not influence the LMS8001 operation when inactive
    rgrCR_f = 1;
    rgrCR_i = 1;
    rgrCPS_f = 1;
    rgrCPS_i = 1;
